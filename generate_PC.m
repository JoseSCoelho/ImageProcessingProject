...
%     end
    ...
    flat_im = reshape(im, [480*640*3, 1]);

    pc_color(:, 1) = flat_im(sub2ind([480, 640, 3], match_coord(2, :), match_coord(1, :), ones(1, length(match_coord))*1));
    pc_color(:, 2) = flat_im(sub2ind([480, 640, 3], match_coord(2, :), match_coord(1, :), ones(1, length(match_coord))*2));
    pc_color(:, 3) = flat_im(sub2ind([480, 640, 3], match_coord(2, :), match_coord(1, :), ones(1, length(match_coord))*3));
...
%     end
    ...
    flat_depth = reshape(depth_array, [480*640, 1]);
    depth_matches(:, 1) = flat_depth(sub2ind([480, 640], match_coord(2, :), match_coord(1, :)));
